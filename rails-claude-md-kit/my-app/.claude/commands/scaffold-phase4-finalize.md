...
最終フェーズ。「最初から動くもの」を完成させる。

## 実行手順

### 1. Seeds の実装

`docs/seeds.md` に従って `db/seeds.rb` を実装。

注意点:
- すべて `find_or_create_by!` で冪等にする（複数回実行しても重複しない）
- 貸出（Lending）を作成する際、書籍の `available_copies` を整合的に更新する
- 「延滞」状態を作るには、`approved` で `due_on` を過去日にし、別途バッチで `overdue` に遷移させる方法を採るか、直接 `overdue` を入れるか、`docs/seeds.md` の意図に沿って実装。簡単のため直接 `overdue` を入れて良い。

`bin/rails db:seed` で投入。エラー時は `db:reset` する前に原因を報告。

### 2. 主要動線のシステムテスト

`docs/screens.md` の主要動線を Capybara で網羅:
...
以下を含む:
- プロジェクト概要（1-2 段落）
- 必要なランタイム（Ruby, PostgreSQL, Node）
- セットアップ手順（`bin/setup`）
- 起動手順（`bin/dev`）
- テストアカウント表（`docs/seeds.md` のアカウント）
- 主要 URL（`/`, `/admin`）
- テスト実行コマンド
