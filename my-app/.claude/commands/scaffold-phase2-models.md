...
`generate model` が生成するファクトリは `nil` / `"MyString"` 等のプレースホルダーが入っている。`db-schema.md` の定義に合わせて書き直すこと（ファイルが既に存在するため Read してから Write する）。`User` ファクトリのメールシーケンスは `@test.local` ドメインにすること（デフォルトフィクスチャの `user1@example.com` 等と衝突してユニーク制約エラーになる）。
