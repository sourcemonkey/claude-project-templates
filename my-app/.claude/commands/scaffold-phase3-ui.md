...
3. **ApplicationController**:
   ...
   - `before_action :authenticate_user!`。root と Devise コントローラは除外する。`HomeController` で `skip_before_action :authenticate_user!, only: :index` を呼ぶ。ApplicationController 側には除外ロジックを書かない
...
10. **Devise 日本語化**: `config.i18n.default_locale = :ja` は Phase 1 で設定済み。`devise-i18n` gem が Rails Engine として i18n 対応ビューと日本語翻訳を自動提供するため、ビュー生成・ERB 編集は不要。`bin/rails generate devise:views` は**実行しない**。実行すると英語ハードコードのビューが `app/views/devise/` に生成され、devise-i18n のビューより優先されて日本語化が壊れる。

## 画面実装の注意

- **検索**: Ransack（`@q = Book.ransack(params[:q])` → `@books = @q.result.page(params[:page])`）
  - `ransackable_attributes` / `ransackable_associations` は Phase 2 で定義済み。未定義なら `docs/db-schema.md` の「Ransack 対応」セクションを参照して追加する
