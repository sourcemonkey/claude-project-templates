...
10. **Devise 日本語化**: `config/application.rb` の `Application` クラス内に `config.i18n.default_locale = :ja` を追加する。`devise-i18n` gem が Rails Engine として i18n 対応ビューと日本語翻訳を自動提供するため、ビュー生成・ERB 編集は不要。`bin/rails generate devise:views` は**実行しない**（実行すると英語ハードコードのビューが `app/views/devise/` に生成され、devise-i18n のビューより優先されて日本語化が壊れる）。
